...
MPIAnalyzer::MPIAnalyzer(const edm::ParameterSet& config) :
	token_(consumes<std::string>(config.getParameter<edm::InputTag>("msg_source"))) {
}

void MPIAnalyzer::analyze(edm::Event const& event, edm::EventSetup const& setup) {
	edm::Handle<std::string> handle;
	event.getByToken(token_, handle);
	MPI_Send(static_cast<void const *>(handle.product()->data()), handle.product()->size(), MPI_CHAR, 1, 0, MPI_COMM_WORLD);
	std::cout << "Buffer sent" << std::endl;
}

void
MPIAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
